 ...
 *
 */
package org.jorlib.demo.frameworks.columngeneration.tspbap.bap.branching;

import org.jgrapht.graph.DefaultWeightedEdge;
import org.jorlib.demo.frameworks.columngeneration.tspbap.bap.branching.branchingDecisions.FixEdge;
import org.jorlib.demo.frameworks.columngeneration.tspbap.bap.branching.branchingDecisions.RemoveEdge;
...
/**
 * Class which creates new branches in the Branch-and-Price tree. This particular class branches on
 * an edge. More precisely, the class checks whether there is a fractional edge in the red resp.
 * blue matchings. The edge with a fractional value closest to 0.5 is selected for branching.
 *
 * @author Joris Kinable
 * @version 22-4-2015
 */
public final class BranchOnEdge
    extends AbstractBranchCreator<TSP, Matching, PricingProblemByColor>
{

    private DefaultWeightedEdge edgeForBranching = null; // Edge to branch on
    private PricingProblemByColor pricingProblemForMatching = null; // Edge is fractional in red or
                                                                    // blue matching
        ...
        for (Matching matching : solution) {
            for (DefaultWeightedEdge edge : matching.edges) {
                ...
                if (edgeValue == null)
                    ...
                    edgeValueMap.get(matching.associatedPricingProblem).put(
                        edge, matching.value + edgeValue);
            }
        }
     ...
     * Create the branches:
     * <ol>
     * <li>branch 1: edge {@code edgeForBranching} must be used by
     * {@code PricingProblemByColor},</li>
     * <li>branch 2: edge {@code edgeForBranching} may NOT used by
     * {@code PricingProblemByColor},</li>
     * </ol>
     * 
    ...
    protected List<BAPNode<TSP, Matching>> getBranches(BAPNode<TSP, Matching> parentNode)
    {
        ...
        BAPNode<TSP, Matching> node2 = this.createBranch(
            parentNode, branchingDecision1, parentNode.getSolution(), parentNode.getInequalities());

        // Branch 2: fix the edge:
        FixEdge branchingDecision2 = new FixEdge(pricingProblemForMatching, edgeForBranching);
        BAPNode<TSP, Matching> node1 = this.createBranch(
            parentNode, branchingDecision2, parentNode.getSolution(), parentNode.getInequalities());
