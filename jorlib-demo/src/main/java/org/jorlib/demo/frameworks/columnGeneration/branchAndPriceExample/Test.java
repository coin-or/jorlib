...
/**
 * Created by jkinable on 5/8/15.
 */
public class Test {

    public Test() {
        Map<String, Integer> instances = new LinkedHashMap<String, Integer>();
        ...
        String[] instanceNames = instances.keySet().toArray(new String[instances.size()]);

            Arrays.sort(instanceNames, new Comparator<String>() {


                @Override
                public int compare(String o1, String o2) {
                    TSPLibInstance problem1 = null;
                    TSPLibInstance problem2 = null;
                    try {
                        problem1 = new TSPLibInstance(new File("./data/tspLib/tsp/" + o1 + ".tsp"));
                        problem2 = new TSPLibInstance(new File("./data/tspLib/tsp/" + o2 + ".tsp"));
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
                    return Integer.compare(problem1.getDimension(), problem2.getDimension());
                }
            });

        for(String s : instanceNames){
            //TSP tsp=new TSP("./data/tspLib/tsp/att48.tsp");
            System.out.println("TSP tsp= new TSP(\"./data/tspLib/tsp/"+s+".tsp\"); //Optimal: "+instances.get(s));
        }
    }
