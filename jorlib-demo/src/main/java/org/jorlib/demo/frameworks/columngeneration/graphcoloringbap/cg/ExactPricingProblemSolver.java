 ...
 * @version 29-6-2016
 */
public final class ExactPricingProblemSolver
    extends
    AbstractPricingProblemSolver<ColoringGraph, IndependentSet, ChromaticNumberPricingProblem>
{
     ...
     */
    public ExactPricingProblemSolver(
        ColoringGraph dataModel, ChromaticNumberPricingProblem pricingProblem)
    {
    ...
    private void buildModel()
    {
        try {
            ...
            for (DefaultEdge edge : dataModel.edgeSet())
                cplex.addLe(
                    cplex.sum(
                        vars[dataModel.getEdgeSource(edge)], vars[dataModel.getEdgeTarget(edge)]),
                    1);
     ...
     */
    @Override
    protected List<IndependentSet> generateNewColumns()
        throws TimeLimitExceededException
    {
        ...
        try {
            ...
            if (!cplex.solve() || cplex.getStatus() != IloCplex.Status.Optimal) {
                if (cplex.getCplexStatus() == IloCplex.CplexStatus.AbortTimeLim) { // Aborted due to
                ...
                } else if (cplex.getStatus() == IloCplex.Status.Infeasible) { // Pricing problem
                                                                              // infeasible
                    ...
                    throw new RuntimeException(
                        "Pricing problem solve failed! Status: " + cplex.getStatus());
                }
            } else { // Pricing problem solved to optimality.
                this.pricingProblemInfeasible = false;
                this.objective = cplex.getObjValue();

                if (objective >= 1 + config.PRECISION) { // Generate new column if it has negative
                                                         // reduced cost
                    double[] values = cplex.getValues(vars); // Get the variable values
                    // Create an Independent Set using all vertices with value 1 in the pricing
                    // problem
                    Set<Integer> vertices = IntStream
                        .range(0, dataModel.getNrVertices())
                        .filter(i -> MathProgrammingUtil.doubleToBoolean(values[i])).boxed()
                        .collect(Collectors.toSet());
                    IndependentSet column =
                        new IndependentSet(pricingProblem, false, this.getName(), vertices, 1);
    ...
    /**
     * Update the objective function of the pricing problem with the new dual information. The dual
     * values are stored in the pricing problem.
     */
    @Override
    ...
    public void branchingDecisionPerformed(BranchingDecision bd)
    {
        try {
            if (bd instanceof SameColor) { // Ensure that two vertices appear together in an
                ...
                IloConstraint branchingConstraint = cplex.addEq(
                    vars[sameColorDecision.vertexPair.getFirst()],
                    vars[sameColorDecision.vertexPair.getSecond()]);
                branchingConstraints.put(sameColorDecision.vertexPair, branchingConstraint);
            } else if (bd instanceof DifferentColor) { // Ensure that two vertices do NOT appear
                                                       // together in an independent set.
                DifferentColor differentColorDecision = (DifferentColor) bd;
                IloConstraint branchingConstraint = cplex.addLe(
                    cplex.sum(
                        vars[differentColorDecision.vertexPair.getFirst()],
                        vars[differentColorDecision.vertexPair.getSecond()]),
                    1);
