...
public final class ExactPricingProblemSolver
    extends AbstractPricingProblemSolver<TSP, Matching, PricingProblemByColor>
{
        ...
        try {
            ...
            for (DefaultWeightedEdge edge : dataModel.edgeSet()) {
                IloIntVar var = cplex.boolVar(
                    "x_" + dataModel.getEdgeSource(edge) + "_" + dataModel.getEdgeTarget(edge));
                vars.put(edge, var);
            }
     ...
     */
    @Override
    protected List<Matching> generateNewColumns()
        throws TimeLimitExceededException
    {
        ...
        try {
            ...
            if (!cplex.solve() || cplex.getStatus() != IloCplex.Status.Optimal) {
                ...
                if (objective >= -pricingProblem.dualCost + config.PRECISION) { // Generate new
                                                                                // column if it has
                                                                                // negative reduced
                                                                                // cost
                    DefaultWeightedEdge[] edges =
                        vars.getKeysAsArray(new DefaultWeightedEdge[vars.size()]); // Get the
                                                                                   // variable
                                                                                   // values
                    IloIntVar[] edgeVarsArray = vars.getValuesAsArray(new IloIntVar[vars.size()]);
                    double[] values = cplex.getValues(edgeVarsArray);

                    Set<DefaultWeightedEdge> matching = new LinkedHashSet<>(); // Generate a new
                                                                               // matching
                    ...
                    Matching column =
                        new Matching("exactPricing", false, pricingProblem, matching, succ, cost);
    ...
    /**
     * Update the objective function of the pricing problem with the new pricing information
     * (modified costs). The modified costs are stored in the pricing problem.
     */
    @Override
        ...
        try {
            if (bd instanceof FixEdge) {
                ...
                if (fixEdgeDecision.pricingProblem == this.pricingProblem)
                    vars.get(fixEdgeDecision.edge).setLB(1); // Ensure that any column returned
                                                             // contains this edge.
                ...
                if (removeEdgeDecision.pricingProblem == this.pricingProblem)
                    vars.get(removeEdgeDecision.edge).setUB(0); // Ensure that any column returned
    ...
    public void branchingDecisionReversed(BranchingDecision<TSP, Matching> bd)
    {
        try {
            if (bd instanceof FixEdge) {
                ...
                RemoveEdge removeEdgeDecision = (RemoveEdge) bd;
                if (removeEdgeDecision.pricingProblem == this.pricingProblem)
                    vars.get(removeEdgeDecision.edge).setUB(1); // Reset the UB to its original
                                                                // value
            }
        } catch (IloException e) {
            e.printStackTrace();
