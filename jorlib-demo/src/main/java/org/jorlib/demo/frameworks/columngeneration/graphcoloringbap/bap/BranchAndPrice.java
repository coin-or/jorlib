...
public final class BranchAndPrice
    extends AbstractBranchAndPrice<ColoringGraph, IndependentSet, ChromaticNumberPricingProblem>
{
    ...
    public BranchAndPrice(
        ...
        List<? extends AbstractBranchCreator<ColoringGraph, IndependentSet,
            ChromaticNumberPricingProblem>> abstractBranchCreators,
        double lowerBoundOnObjective, double upperBoundOnObjective)
    {
        super(
            dataModel, master, pricingProblem, solvers, abstractBranchCreators,
     ...
     */
    @Override
    protected List<IndependentSet> generateInitialFeasibleSolution(
        BAPNode<ColoringGraph, IndependentSet> node)
    {
        List<IndependentSet> artificialSolution = new ArrayList<>();
        for (int v = 0; v < dataModel.getNrVertices(); v++) {
            artificialSolution.add(
                new IndependentSet(
                    pricingProblems.get(0), true, "Artificial",
                    new HashSet<>(Collections.singletonList(v)), objectiveIncumbentSolution));
        }
        return artificialSolution;
    }

    /**
     * Checks whether the given node is integer. A solution is integer if every vertex is contained
     * in exactly 1 independent set, that is, if every vertex is assigned a single color.
     * 
