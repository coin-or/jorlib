 ...
 * Given a knapsack constraint: {@code \sum_i a_i x_i <= b} where {@code x_i} are binary variables,
 * {@code b} a positive integer and {@code a_i} integer coefficients. Let {@code N} be the set of
 * variables in the knapsack constraint. For a given assignment of values to the variables, this
 * class computes two types of violated cover inequalities:
 * <ol>
 * <li>Minimal covers: {@code \sum_{j\in C} \leq |C|-1}, where {@code C\subseteq N, \sum_{j\in C}
 * a_i >b}</li>
 * <li>Lifted covers: {@code \sum_{j\in N\setminus C} \alpha_jx_j + \sum_{j\in C2} \gamma_jx_j +
 * \sum_{j\in C1} x_j \leq |C1|-1+\sum_{j\in C2}\gamma_j}, where {@code C1 \cap C2= \emptyset},
 * {@code C1 \cup C2= C}, {@code C} a minimal cover as defined above.</li>
 * </ol>
 * <br>
 *
 * NOTE: Separating violated Lifted Cover Inequalities is NP-hard. Hence we rely on lifting and a
 * separation heuristic. First we attempt to find a violated Lifted Cover inequality with
 * {@code C2=\emptyset}. If we can't find such an inequality, we set {@code C2} to: {@code C2={k},
 * k=arg max_{j\in C}a_j variableValue[j]}, and {@code C1=C\setminus C2}, and retry.<br>
 * <br>
 * 
...
public class LiftedCoverInequalitySeparator
{
    ...
    /** COVER INEQUALITIES **/
    private boolean coverInequalityExists; // Indicates whether a cover inequality exists. If {@code
                                           // \sum_i a_i \leq b}, then no cover exists and hence no
                                           // inequality can be generated.

    /** Minimal cover **/
    private int minimalCoverRHS; // Right hand side of the minimal cover inequality
    private boolean[] minimalCover; // Boolean array indicating whether a variable is in the minimal
                                    // cover
    private Set<Integer> minimalCoverSet; // Set containing the variables which are part of the
                                          // minimal cover
    private boolean minimalCoverIsViolated; // Returns true if LHS > minimalCoverSize-1 for the
                                            // given variableValues
    ...
    /**
     * Given a knapsack constraint: {@code \sum_{i=0}^n a_ix_i \leq b} This method separates minimal
     * Cover Inequalities, i.e it will search for a valid cover {@code \sum_{i\in C} x_i \leq |C|-1}
     ...
     */
    public void separateMinimalCover(
        int nrVars, int[] knapsackCoefficients, int b, double[] variableValues)
    {
        this.nrVars = nrVars;
    ...
    /**
     * Given a knapsack constraint: {@code \sum_{i=0}^n a_ix_i \leq b} This method separates Lifted
     * Cover Inequalities, i.e it will search for a valid lifted cover inequality {@code \sum_{j\in
     * N\setminus C} \alpha_jx_j + \sum_{j\in C2} \gamma_jx_j + \sum_{j\in C1} x_j \leq
     * |C1|-1+\sum_{j\in C2}\gamma_j} which is violated by the current variable values, where
     * {@code C1 \cap C2= \emptyset, C1 \cup C2= C, C} a minimal cover
     * 
     * @param nrVars number of variables in the knapsack constraint
     * @param knapsackCoefficients a_i
     * @param b right hand side of the knapsack constraint
     * @param variableValues values of the x_i variables
     * @param performDownLifting When set to true, additional effort is performed to find a violated
     *        Lifted Cover AbstractInequality. When this value is false, C2 will be an empty set.
     * 
     */
    public void separateLiftedCover(
        int nrVars, int[] knapsackCoefficients, int b, double[] variableValues,
        boolean performDownLifting)
    {
        ...
        if (!coverInequalityExists)
            return;
        // Which we then try to lift
        // PART 1: Lifting heuristic to produce a lifted cover inequality with empty C2, see
        // (Lifting heuristic, p461)
        this.computeLiftedCover(Collections.emptySet());
        if (liftedCoverIsViolated) // If we found a violated cover inequality, return, else continue
                                   // with part 2
            return;
        // PART 2: Lifting heuristic to produce a lifted cover inequality with C2={k}, see
        // (Separation algorithm, p462)
        if (performDownLifting)
            this.computeLiftedCoverWithNonEmptyC2();
    }

    /**
     * Compute a minimal cover by solving: {@code S=min \sum_{i=0}^n (1-variableValues[i]) z_i}<br>
     * {@code s.t. \sum_{i=0}^n a_iz_i \geq b+1} <br>
     * {@code z_i binary}<br>
     * <br>
     * 
     * if {@code S<1}, the cover is violated.<br>
     * <br>
     * 
     * Note: Instead of solving the above problem, we transform it into a knapsack problem by
     * substituting {@code z_i=1-y_i}, i.e. we solve:<br>
    ...
    private void computeMinimalCover()
    {
        ...
        knapsackAlgorithm
            .solveKnapsackProblem(nrVars, maxKnapsackWeight, itemValues, knapsackCoefficients);
    ...
    private void computeLiftedCover(Set<Integer> C2)
    {
        ...
        // List<Integer> cover=new ArrayList<Integer>(); //Variable ids which are part of the cover
        // C
        Set<Integer> Lk = new LinkedHashSet<>(); // Variable ids of N/C, where N is the complete set
                                                 // of vars.
        ...
        while (!Lk.isEmpty()) {
            ...
            for (int j : Lk) { // Calculate Beta_j for j\in Lk, equation (2.4), p462
                ...
                double knapsackValue = knapsackAlgorithm.solveKnapsackProblem(
                    NminLk.size(), maxKnapsackWeight, itemValues, itemWeights);
        ...
        // Lk should be empty here. Test whether \sum_j\in N \alpha_j variableValues[j] > |C| -1, if
        // so, we found a violated inequality \sum_j\in N \alpha_j variableValues[j] <= |C| -1
    ...
    private void computeLiftedCoverWithNonEmptyC2()
    {
        ...
        itemValues[k] = 0;
        double knapsackValue = knapsackAlgorithm.solveKnapsackProblem(
            nrVars, b + knapsackCoefficients[k], itemValues, knapsackCoefficients);
    ...
    /**
     * Indicates whether a cover inequality exists, i.e returns whether {@code \sum_i a_i > b}. You
     * should NOT ask for a minimal or lifted cover if this function returns false!
     * 
     * @return Indicates whether a cover inequality exists, i.e returns whether
     *         {@code \sum_i a_i > b}. You should NOT ask for a minimal or lifted cover if this
    ...
    /**
     * Returns an boolean array indicating which variables belong to the minimal cover inequality C:
     * {@code\sum_{i\in C} x_i <= |C|-1}, i.e. minimalCover[i] is true if variable i is in the
     * cover.
     * 
     * @return an boolean array indicating which variables belong to the minimal cover inequality C:
     *         {@code\sum_{i\in C} x_i <= |C|-1}, i.e. minimalCover[i] is true if variable i is in
     *         the cover.
     */
