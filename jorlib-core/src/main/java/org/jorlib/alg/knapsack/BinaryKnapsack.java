 ...
 * {@code x_i binary}<br>
 * 
 * The implementation is memory efficient: it does not rely on large matrices. The knapsack problem
 * is solved as a binary tree problem. Each level of the tree corresponds to a specific item. Each
 * time we branch on a particular node, two child-nodes are created, reflecting whether the item at
 * the level of the child nodes is selected, or not. As a result, at most 2^n nodes, where n is the
 * number of items, are created. In practice, the number of generated nodes is significantly
 * smaller, as the number of items one can choose depends on the knapsack weight. Furthermore, the
 * search tree is pruned using bounds.
 * <p>
 * Consider replacing this implementation by a faster one such as MT2<br>
 * <br>
 * 
 * NOTE: All item weights, as well as the maxKnapsackWeight have to be integers. The item weights
 * can be fractional, both positive and negative. Obviously, since this is a maximization problem,
 * items with a value smaller or equal to 0 are never selected.
 * 
 *
    ...
    implements KnapsackAlgorithm
{
    ...
    /**
     * Calculates a greedy solution for the knapsack problem. This solution is a valid lower bound
     * and is used for pruning.
     * 
     * @param itemOrder Order in which the items are considered by the greedy algorithm. The items
     *        are sorted ascending, based on their value/weight ratio
     */
     ...
     */
    public double solveKnapsackProblem(
        int nrItems, int maxKnapsackWeight, double[] itemValues, int[] itemWeights)
    {
        ...
        // Define the order in which items will be processed. The items are sorted based on their
        // value/weight ratio, thereby considering proportionally more valuable items first.
        ...
        Arrays.sort(itemOrder, new Comparator<Integer>()
        {
            ...
            public int compare(Integer item1, Integer item2)
            {
                return -1 * Double.compare(
                    itemValues[item1] / itemWeights[item1], itemValues[item2] / itemWeights[item2]);
            }
        });
        ...
        while (!queue.isEmpty()) {
            ...
            if (kn.bound > bestValue && kn.level < nrItems - 1) {
                kn.level++;
                // Create 2 new nodes, one where item <itemToAdd> is used, and one where item
                // <itemToAdd> is skipped.
                int itemToAdd = itemOrder[kn.level];
                if (kn.weight + itemWeights[itemToAdd] <= maxKnapsackWeight
                    && itemValues[itemToAdd] > 0)
                { // Check whether we can add the next item and whether its value is positive
                    KnapsackNode knCopy = kn.copy();
                    knCopy.addItem(itemToAdd, itemWeights[itemToAdd], itemValues[itemToAdd]);
                    knCopy.bound = calcBound(
                        itemOrder, knCopy.level + 1, maxKnapsackWeight - knCopy.weight,
                        knCopy.value);
    ...
    private double calcBound(Integer[] itemOrder, int level, int remainingSize, double value)
    {
        ...
        if (level < nrItems) {
            bound += itemValues[itemOrder[level]]
                * (remainingSize / (double) itemWeights[itemOrder[level]]);
        }
        return bound;
    }
    ...
    /**
     * Knapsack nodes represent partial solutions for the knapsack problem. A subset of the
     * variables, starting from the root node of the tree up to <level> level, have been fixed.
     * 
     * @author jkinable
     *
     */
    private final class KnapsackNode
        implements Comparable<KnapsackNode>
    {
        ...
        public KnapsackNode(
            int nrItems, int level, double bound, double value, int weight, boolean[] selectedItems)
        {
            this.nrItems = nrItems;
            this.level = level;
            this.bound = bound;
        ...
        public KnapsackNode copy()
        {
            ...
            return new KnapsackNode(
                nrItems, this.level, this.bound, this.value, this.weight, selectedItemsCopy);
        }
        ...
        {
            return "Level: " + level + " bound: " + bound + " value: " + value + " weight: "
                + weight + " items: " + Arrays.toString(selectedItems) + " \n";
        }
    }
}
