...
/**
 * Stores the nodes (by their identifier) that are visited in a tour. Tours are cyclic, so an
 * implicit edge exists between the last index and the first.
 * 
 * @author David Hadka
 */
public class Tour
{
    ...
    /**
     * Returns the identifier of the node visited at the specified index. Tours are cyclic, so
     * specifying an index {@code < 0} or {@code >= size()} is cycled through the tour, never
    ...
    /**
     * Sets the identifier of the node visited at the specified index. Tours are cyclic, so setting
     * an index {@code < 0} or {@code >= size()} is cycled through the tour, never causing an
    ...
    /**
     * Calculates and returns the total distance of this tour. The total distance includes the
     * distance from the last node back to the first node in the tour.
     * 
    ...
    /**
     * Returns {@code true} if this tour contains all the fixed edges required by the TSPLIB problem
     * instances; {@code false} otherwise.
     * 
     * @param problem the TSPLIB problem instance this tour is a solution for
     * @return {@code true} if this tour contains all the fixed edges required by the TSPLIB problem
     *         instances; {@code false} otherwise
     */
    ...
    /**
     * Returns {@code true} if this tour is a Hamiltonian cycle; {@code false} otherwise. A
     * Hamiltonian cycle is a path through a graph that visits every node exactly once.
     * 
     * @param problem the TSPLIB problem instance this tour is a solution for
    ...
    /**
     * Reverses the order in which the nodes are visited between the two specified indices. The
     * first index defines the start of the reverse operation; the second index defines the stopping
     * position. Tours are cyclic, so the reverse operation, when the second index is less than the
     * first, gets wrapped around the end of the array. The indices are inclusive.
     * 
    ...
    /**
     * Returns {@code true} if this tour is equivalent to the specified tour; {@code false}
     * otherwise. Two tours are considered equivalent if they visit the same nodes in the same
     * order. This comparison ignores the direction of the node traversal.
     * 
     * @param other the tour that is being compared
     * @return {@code true} if this tour is equivalent to the specified tour; {@code false}
     *         otherwise
     */
    ...
    /**
     * Returns the canonical tour with the given length. The canonical tour visits the nodes in
     * order, i.e., {@code [1, 2, 3, ..., length]}.
     * 
    ...
    /**
     * Returns a random tour with the given length. A random tour will visit every node exactly
     * once.
     * 
     * @param length the number of nodes in the resulting tour
    ...
    /**
     * Constructs a tour from the specified array. The values in the array should start at
     * {@code 1}.
     * 
