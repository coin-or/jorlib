...
/**
 * The CutHandler is a manager class which maintains various {@link AbstractCutGenerator}(s) to
 * generate inequalities of different types.
 * 
...
public class CutHandler<T extends ModelInterface, W extends MasterData<T, ? extends AbstractColumn<T, ?>, ? extends AbstractPricingProblem<T, ?>, ? >>
{
    ...
    /**
     * Supply the data object containing the data from the master problem which is required by the
     * cutGenerators to separate valid inequalities.
     *
     ...
     * {@link AbstractCutGenerator#generateInqualities() generateInequalities} method for each of
     * the registered cut generators. Returns as soon as a single cut has been found!
     * 
     * @return true if inequalities have been found
     */
    public boolean generateInequalities()
    {
        List<AbstractInequality<T, W>> separatedInequalities = new ArrayList<>();
    ...
    /**
     * Add a set of inequalities from an external source. Can be used to add a number of initial
     * inequalities which are not separated from the master problem.
     * 
    ...
    public void addCuts(Collection<AbstractInequality<T, W>> cuts)
    {
        ...
        for (AbstractInequality<T, W> cut : cuts) {
            if (!this.cutGenerators.contains(cut.maintainingGenerator))
                throw new RuntimeException(
                    "Attempt to add cut failed. AbstractCutGenerator for this type of inequalities is not registered with the cut handler!");
            else
    ...
    {
        List<AbstractInequality<T, W>> cuts = new ArrayList<>();
        for (AbstractCutGenerator<T, W> cutGen : cutGenerators)
    ...
    protected class CHNotifier
    {
        ...
        /**
         * Fires a StartGeneratingCutsEvent to indicate that the cut handler starts generating
         * inequalities
         */
        ...
        /**
         * Fires a FinishGeneratingCutsEvent to indicate that that the cut handler finished
         * generating inequalities
         * 
         * @param separatedInequalities list of newly separated inequalities which have been
         *        generated
         */
        public void fireFinishGeneratingCutsEvent(List<AbstractInequality<T, W>> separatedInequalities)
        {
            FinishGeneratingCutsEvent finishGeneratingCutsEvent = null;
            for (CHListener listener : listeners) {
                if (finishGeneratingCutsEvent == null)
                    finishGeneratingCutsEvent = new FinishGeneratingCutsEvent(
                        CutHandler.this, Collections.unmodifiableList(separatedInequalities));
