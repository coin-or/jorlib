...
/**
 * Defines a Pricing Problem. Often, Column generation models decouple in a single Master problem
 * and one or more Pricing problems. The pricing problems can be solved independently. This class
 * models a pricing problem and provides storage for information coming from the master problem
 * which is required to solve the pricing problems, e.g. dual values.
 * 
...
public abstract class AbstractPricingProblem<T extends ModelInterface,
                                             U extends AbstractColumn<T, ? extends AbstractPricingProblem<T, U, W>>,
                                             W extends MasterData<T, U, ? extends AbstractPricingProblem<T, U, W>, ?>>
    implements BranchingDecisionListener<T, U, W>
{
    ...
    /**
     * Store dual information in the dualCosts array and dualCost variable. The pricing problem
     * often looks like: {@code a_1x_1+a_2x_2+...+a_nx_n <= b}, where {@code a_i} are dual
     * variables, and {@code b} some constant. The dualCosts array would hold the {@code a_i} values
     * whereas {@code b} is stored in the dualCost variable
     *
     * @param dualCosts dual values
