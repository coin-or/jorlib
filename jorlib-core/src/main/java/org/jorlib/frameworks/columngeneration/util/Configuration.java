...
public class Configuration
{
    ...
    /**
     * Use configuration from file. The properties file can contain one or more of the fields
     * mentioned. Fields not included in the properties file will default to their default values.
     * 
    ...
    protected Configuration(Properties properties)
    {
        ...
        // Global params
        MAXTHREADS = (properties.containsKey("MAXTHREADS")
            ? Integer.valueOf(properties.getProperty("MAXTHREADS")) : 4);
        PRECISION = (properties.containsKey("PRECISION")
            ? Double.valueOf(properties.getProperty("PRECISION")) : 0.000001);

        // CG & Branch-and-price params
        CUTSENABLED = (properties.containsKey("CUTSENABLED")
            ? Boolean.valueOf(properties.getProperty("CUTSENABLED")) : true);
        EXPORT_MODEL = (properties.containsKey("EXPORT_MODEL")
        ...
        QUICK_RETURN_AFTER_CUTS_FOUND = (properties.containsKey("QUICK_RETURN_AFTER_CUTS_FOUND")
            ? Boolean.valueOf(properties.getProperty("QUICK_RETURN_AFTER_CUTS_FOUND")) : true);
    }
    ...
    /**
     * The {@link CutHandler} invokes the {@link AbstractCutGenerator}(s) one by one to generate
     * inequalities. When a particular cutGenerator does not yield any inequalities, the cutHandler
     * will move on to the next registered cutGenerator. When quickReturnAfterCutsFound is set to
     * true, the cutHandler will return as soon as any inequalities have been found. When set to
     * false, all cutGenerators will be invoked. Default: true
     */
