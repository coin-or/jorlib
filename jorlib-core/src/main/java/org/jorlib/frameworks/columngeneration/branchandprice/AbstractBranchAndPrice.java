    ...
    U extends AbstractColumn<T, V>, V extends AbstractPricingProblem<T, U>>
{
    ...
    /**
     * Counter used to provide a unique ID for each node (counter gets incremented each time a new
     * node is created)
     **/
    ...
    /**
     * Creates a new Branch-and-Price instance, thereby initializing the data structures, and the
     * root node.
     * 
        ...
        double upperBoundOnObjective)
    {
        ...
        for (Class<? extends AbstractPricingProblemSolver<T, U, V>> solverClass : solvers) {
            ...
            PricingProblemBundle<T, U, V> bunddle =
        ...
        // Add the master problem and the pricing problem solver instances as
        // BranchingDecisionListeners
        ...
        // Create a new notifier which informs associated listeners about events occurring the the
    ...
    {
        this(
            dataModel, master, Collections.singletonList(pricingProblem), solvers, branchCreators,
            lowerBoundOnObjective, upperBoundOnObjective);
    }

    /**
     * Provide an initial solution. This solution will be used as an initial set of columns for the
     * master problem of the root node
     * 
    ...
    public void warmStart(int objectiveInitialSolution, List<U> initialSolution)
    {
        ...
        if (rootNode.nodeID != 0)
            throw new RuntimeException(
                "This method can only be invoked at the start of the Branch-and-Price procedure, before runBranchAndPrice is invoked");
    ...
    /**
     * Starts running the Branch-and-Price algorithm. Note: In the current version of the code, one
     * should not invoke this function multiple times on the same instance!
     * 
    ...
    public void runBranchAndPrice(long timeLimit)
    {
        ...
        while (!queue.isEmpty()) {
            ...
            // Prune this node if its bound is worse than the best found solution. Since all
            // solutions are integral, we may round up/down, depending on the optimization sense
            ...
            // Generate an initial solution for this node to guarantee that the master problem is
            // feasible
            if (bapNode.nodeID != 0) {
     ...
     */
    protected void solveBAPNode(BAPNode<T, U> bapNode, long timeLimit)
        ...
        try {
            ...
            if (cg != null) {
                ...
                notifier.fireFinishCGEvent(
                    bapNode, cg.getBound(), cg.getObjective(), cg.getNumberOfIterations(),
    ...
    /**
     * Returns a unique node ID. The internal nodeCounter is incremented by one each time this
     * method is invoked.
     * 
     * @return returns a unique node ID for the purpose of creating new BAPNodes, thereby
     *         guaranteeing that none of the nodes in the Branch-and-Price tree have this ID.
     */
    ...
    /**
     * Returns the strongest available bound after the root node has been solved. Whenever the root
     * node was solved to optimality, the value returned equals the objective of (optimal solution)
     * of the root node. Whenever the node is not solved to optimality, e.g. due to a time limit,the
     * strongest lower bound (minimization problem) or strongest upper bound (maximization problem)
     * is returned. Not that this function is equivalent to solving the master problem relaxation
    ...
    /**
     * Returns whether the solution is optimal, that is, whether the entire Branch-and-Price tree
     * has been processed
     * 
     * @return {@code true} if the problem instance has been solved to optimality. ({@code getBound}
     *         and {@code getObjective} methods must yield the same value.
     */
    ...
    /**
     * Returns strongest available bound on the objective function. If the problem is a minimization
     * problem, the strongest available lower bound is returned, if the problem is a maximization
     * problem, the strongest available upper bound is returned
     * 
     * @return Returns the best bound on the optimal solution (upper bound if the master is a
     *         maximization problem, a lower bound if the master is a minimization problem)
     */
    public double getBound()
    {
        return (optimizationSenseMaster == OptimizationSense.MINIMIZE ? this.lowerBoundOnObjective
            : this.upperBoundOnObjective);
    }
     ...
     * 
     * @return total time spent solving the Branch-and-Price problem. This time should equal
     *         {@link #getMasterSolveTime()}+{@link #getPricingSolveTime()}+overhead due to
     *         branching;
     */
    ...
    /**
     * Counts how many column generation iterations have been made over the entire Branch-and-Price
     * tree
     * 
     * @return returns the total number of column generation iterations (summed over all processed
     *         nodes)
     **/
    ...
    /**
     * Create an artificial solution which satisfies the node's master problem and hence constitutes
     * a feasible initial solution. The columns are not necessary feasible or meet the definition of
     * a column; it is undesirable that these columns end up in a final solution. To prevent them
     * from ending up in a final solution, a high cost is associated with them.
     * 
     ...
     * Whenever a node in the Branch-and-Price tree is being solved, its initial master problem
     * needs to be feasible. To ensure feasibility, a minimal subset of initial columns needs to be
     * provided. This initial subset is generated in this method.
     *
     * Any node, other than the root node, will receive a set of initial columns from its parent
     * node. However, because a number of columns which did not comply with the branching decision
     ...
     * 'artificial' columns. These columns are not necessary feasible columns (they may not satisfy
     * the definition of a feasible column as defined by the pricing problem). Furthermore, these
     * columns may also violate branching decisions. To prevent artificial columns from ending up in
     * a final solution, a high cost is associated with them (higher than any solution consisting of
     * normal columns). Whenever a cheaper feasible solution is available, the Master Problem will
    ...
    /**
     * Define how the nodes in the Branch-and-Price tree are processed. By default, the tree is
     * processed in a Depth-First-Search manner but any other (custom) approach may be specified.
     * This method may also be invoked during the search. The nodes already present in the queue
    ...
    /**
     * Destroy both the master problem and pricing problems. A CutHandler which has been provided to
     * the Constructor will not be destroyed by this method.
     */
    public void close()
    {
        master.close();
    ...
    protected class BAPNotifier
    {
         ...
         * @param numberOfCGIterations Number of Column Generation iterations it took to solve the
         *        node
