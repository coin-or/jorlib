...
public class CutHandler<T extends ModelInterface,W extends MasterData>{
	...
	/**
	 * Generates cuts for the master problem of the column generation model, by invoking the {@link AbstractCutGenerator#generateInqualities() generateInequalities} method for each
	...
	public void addCuts(Collection<AbstractInequality> cuts){
		...
		for(AbstractInequality cut : cuts){
			if(!this.cutGenerators.contains(cut.maintainingGenerator))
				throw new RuntimeException("Attempt to add cut failed. AbstractCutGenerator for this type of cuts is not registered with the cut handler!");
			else
